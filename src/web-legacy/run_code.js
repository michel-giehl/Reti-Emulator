...
compile_picoc = async (code) => {
    let response = await fetch("/compile", {method: "POST", headers: {"Language": "picoc"}, body: code})
    const text = await response.text()
    if (response.ok) {
        json = JSON.parse(text)
...
compile_reti = async (code) => {
    let response = await fetch("/compile", {method: "POST", headers: {"Language": "reti"}, body: code})
    const text = await response.text()
    if (response.ok) {
        console.log(text)
    }
